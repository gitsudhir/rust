...
You can configure the API endpoint and other parameters using environment variables at compile time:

1. `ULTRASONIC_API_ENDPOINT` - The URL to send ultrasonic data to (default: "http://sudhirkumar.in/api/ultrasonic")
2. `ULTRASONIC_API_TIMEOUT` - Request timeout in seconds (default: 10)
3. `ULTRASONIC_API_MAX_RETRIES` - Maximum number of retry attempts (default: 3)

Example build command with custom API endpoint:
```bash
ULTRASONIC_API_ENDPOINT="http://your-api.com/ultrasonic" cargo build --release
```
